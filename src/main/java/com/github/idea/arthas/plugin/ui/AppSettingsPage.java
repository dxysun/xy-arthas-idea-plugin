...
public class AppSettingsPage implements Configurable {
    ...
    /**
     * watch/tt 观察表达式 入参变量名
     */
    private JTextField watchParamsVariableTextField;
    /**
     * watch/tt 观察表达式 异常变量名
     */
    private JTextField watchThrowVariableTextField;
    ...
    public boolean isModified() {
        boolean modify = !springContextStaticOgnlExpressionTextFiled.getText().equals(settings.staticSpringContextOgnl)
                ...
                || !watchParamsVariableTextField.getText().equals(settings.watchParamsVariable)
                || !watchThrowVariableTextField.getText().equals(settings.watchThrowVariable)
                || tableModify
    ...
    private void saveSettings() {
        ...
        this.saveWatchExpressVariableSetting();
    ...
    /**
     * 保存 watch/tt 观察表达式变量名配置
     */
    private void saveWatchExpressVariableSetting() {
        String watchParamsVariableText = watchParamsVariableTextField.getText();
        String watchThrowVariableText = watchThrowVariableTextField.getText();
        if (StringUtils.isBlank(watchParamsVariableText)) {
            watchParamsVariableText = ArthasCommandConstants.DEFAULT_WATCH_PARAMS_VARIABLE;
        }
        if (StringUtils.isBlank(watchThrowVariableText)) {
            watchThrowVariableText = ArthasCommandConstants.DEFAULT_WATCH_THROW_VARIABLE;
        }
        settings.watchParamsVariable = watchParamsVariableText.trim();
        settings.watchThrowVariable = watchThrowVariableText.trim();
        PropertiesComponentUtils.setValue("watchParamsVariable", settings.watchParamsVariable);
        PropertiesComponentUtils.setValue("watchThrowVariable", settings.watchThrowVariable);
    }
    ...
    private void loadSettings() {
        ...
        watchParamsVariableTextField.setText(settings.watchParamsVariable);
        watchThrowVariableTextField.setText(settings.watchThrowVariable);
        initEvent();
    }
