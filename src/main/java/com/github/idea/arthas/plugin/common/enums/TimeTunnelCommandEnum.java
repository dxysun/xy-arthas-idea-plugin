...
public enum TimeTunnelCommandEnum implements EnumCodeMsg<String> {
    ...
    WATCH_EXPRESS("tt -w '{method.name," + ShellScriptVariableEnum.WATCH_PARAMS.getCode() + ",returnObj," + ShellScriptVariableEnum.WATCH_THROW.getCode() + "}' -x 3 -i 1000", "观察表达式入参返回值 -w, -watch-express( watch the time fragment by ognl express)"),
